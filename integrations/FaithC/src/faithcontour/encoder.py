        ...
        for level in range(min_level, self.octree.max_level + 1):
            ...
            if level < self.octree.max_level:
                ...
                for i in range(0, current_ijk.shape[0], chunk_size):
                    ...
                    has_clip_payload = (
                        hasattr(result, 'aabb_ids') and result.aabb_ids is not None and result.aabb_ids.numel() > 0
                        and hasattr(result, 'face_ids') and result.face_ids is not None
                        and hasattr(result, 'centroids') and result.centroids is not None
                        and hasattr(result, 'areas') and result.areas is not None
                    )
                    if has_clip_payload:
